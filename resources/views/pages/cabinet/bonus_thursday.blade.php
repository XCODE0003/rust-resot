...
@section('title', __('Бонусный подарок') . ' Thursday - ' . config('options.main_title_'.app()->getLocale(), '') )

@section('content')

    <div class="inner-header">{{ __('Бонусный подарок') }} Thursday</div>
                ...
                <section class="open_cases-area">
                    <div class="content-area">
                        <div class="title-big title-big-type-1 flex-sc">
                            ...
                            <h2 id="win_item-title"><span>{{ __('Бонусный подарок') }} Thursday</span></h2>
                        ...
                        <div id="roll" class="open_cases-all-case-scroll">
                            ...
                            <div class="open_cases-item-case main_case active flex-cs">
                                <img src="/images/case_thursday.png" alt="case_thursday">
                            </div>
                        ...
                        <div class="open_cases-link-block flex-sc">
                            ...
                            <div class="flex-sbc open_cases-links">
                                ...
                                <div @if(isset(auth()->user()->steam_trade_url) && strlen(auth()->user()->steam_trade_url) > 10) id="opencase" @else id="set-tradeurl" @endif class="opencase-items flex-sbc open_cases-link active @if(isset(auth()->user()->online_time_thursday) && getHoursAmount(auth()->user()->online_time_thursday) >= config('options.bonusm_online_amount', '100')) @else disable @endif">
                                    <div class="open_cases-link-image"><img src="/images/icons/open-case_icon.png" alt=""></div>
                                    <div class="flex-cs">
                                        @if(isset(auth()->user()->online_time_thursday) && getHoursAmount(auth()->user()->online_time_thursday) >= config('options.bonusm_online_amount', '100')) {{ __('Выиграть подарок') }} @else {{ __('Не доступно') }} @endif
                                    </div>
                                ...
                                <div class="open_cases-link-bar flex-cc">
                                    <div class="flex-sbc open_cases-link-take">
                                        <div @if(isset(auth()->user()->online_time_thursday) && getHoursAmount(auth()->user()->online_time_thursday) >= config('options.bonusm_online_amount', '100')) id="take-bonus" @else id="disable" @endif class="flex-cs">{{ __('ЗАБРАТЬ ПОДАРОК') }}</div>
                                        <div class="open_cases-link-shadow"></div>
                                    </div>
                                </div>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('bonus_thursday.getBonusItemsForRoll') }}",
                dataType: "json",
