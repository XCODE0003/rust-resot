@extends('layouts.main')

@section('title', __('Мой Профиль') . ' - ' . config('options.main_title_' . app()->getLocale(), ''))

@push('head')
    <link rel="stylesheet" href="/css/trading.css?ver=1.1">
@endpush

@section('content')

    <!-- VIP -->
    @if(!empty($services))
        @foreach($services as $name => $service)

            <div id="{{ $name }}" class="vip__popup">
                <div class="vip-popup__close"></div>
                <div class="vip-popup__content">
                    <!-- close -->
                    <div class="vip-popup-content__close" onclick="$('#{{ $name }}').hide();">
                        <i class="fa-solid fa-xmark"></i>
                    </div>
                    <!-- content -->
                    <div class="vip-popup__content--in">

                        <div class="vip-popip-content-in__title">{{ $name }}</div>
                        <div class="trading-user-info">

                            @if(!empty($service) && count($service) > 0)
                                @foreach($service as $serv)

                                    <div class="vip-group">
                                        <span>{{ __('Сервер') }}: {{ getserver($serv->server_id)->name }} | {{ $name }} {{ __('активен до') }} {{ $serv->date }}</span>
                                        <a href="{{ route('shop.item.show', $serv->server_id) }}" class="shop-pick-content-button">{{ __('Продлить') }} {{ $name }}</a>
                                    </div>
                                @endforeach
                            @else
                                <div class="vip-group" style="width: 35%">
                                    <span>{{ __('У Вас не активен') }} {{ str_replace('ElitePack', 'Elite Pack', $name) }}</span>
                                    <a href="{{ route('shop') }}" class="shop-pick-content-button">{{ __('Купить') }} {{ $name }}</a>
                                </div>
                            @endif
                        </div>
                    </div>
                </div>
            </div>
        @endforeach
    @endif


    <!-- INFO -->
    <div class="info__popup">
        <div class="info-popup__close"></div>
        <div class="info-popup__content">
            <!-- close -->
            <div class="info-popup-content__close">
                <i class="fa-solid fa-xmark"></i>
            </div>
            <!-- content -->
            <!-- content -->
            <div class="info-popup__content--in">
                <div class="info-popup-content-in__text">
                    Эта ссылка нужна для получения подарка Вы должны указать ссылку для
                    обмена STEAM. <a href="#">Где взять ссылку?</a>
                </div>
            </div>
        </div>
    </div>
    <!-- INFO -->



    <div class="inner">
        <div class="container">

            <div class="trading">
                <div class="trading-link">
                    <div class="trading-user">
                        <!-- account box -->
                        <div class="trading-user__account--box">
                            <!-- item -->
                            <div class="trading-fp">
                                <img src="{{ auth()->user()->avatar }}" alt="avatar">
                            </div>
                            <!-- item -->
                            <div class="trading-user-account-box__right">
                                <h1 class="trading-user__nick">{{ auth()->user()->name }}</h1>
                                <div class="tuabr__box d-flex">
                                    @if(!empty($services))
                                        @foreach($services as $name => $service)
                                            <a href="#" onclick="$('#{{ $name }}').show();" class="trading-user-account-box-right__vip-btn">{{ $name }}</a>
                                        @endforeach
                                    @endif
                                </div>

                            </div>
                        </div>

                        <!-- item -->
                        <div class="personal-info personal-info-id-flex">
                            <div class="personal-info__id--nums">
                                <div class="personal-info__id">Steam ID: </div>
                                <div class="personal-info__id--nums--text">{{ auth()->user()->steam_id }}</div>
                            </div>

                            <div class="personal-info__id--nums--icon steamid-copy" data-steamid="{{ auth()->user()->steam_id }}">
                                <img src="/images/id-copy-icon.png" alt="icon">
                            </div>

                        </div>
                        <!-- item -->
                        <div class="personal-info personal-info-balance">
                            <div class="personal-info-balance__box ">
                                <div class="personal-info-balance">{{ __('Баланс') }}: </div>
                                <span class="account-balance">{{ getCurrentUserBalance() }}</span>
                            </div>
                            <!-- item -->
                            <a id="topup" class="shop-pick-content-button">
                                <div class="shop-pick-content-button-f">
                                    +
                                </div>
                                <div class="shop-pick-content-button-f">
                                    {{ __('Пополнить') }}
                                </div>
                            </a>
                        </div>
                        <a href="{{ route('account.delivery_requests') }}" class="shop-pick-content-button--sp">
                            <div class="shop-pick-content-button--sp--text">
                                {{ __('Заявки на вывод предметов') }}
                            </div>
                        </a>
                        <a href="{{ route('promocode') }}" class="personal-info__promocode">{{ __('Активировать промокод') }}</a>
                    </div>
                    <!-- // trading-user end -->
                    <div class="trading-info">

                        <form action="{{ route('account.profile.setTradeUrl') }}" method="POST">
                            @csrf
                            <div class="trading-link-top">

                                <div class="trading-link-text">{{ __('Укажите ссылку Steam для обмена') }}</div>

                                <div class="trading-link-copy__link">
                                    <input type="text" id="trading-link" name="steam_trade_url" @if(strlen(auth()->user()->steam_trade_url) > 10) value="{{ auth()->user()->steam_trade_url }}" @else value="" @endif autocomplete="off" required />
                                    @if(strlen(auth()->user()->steam_trade_url) > 10)
                                        <i class="fa-solid fa-circle-check"></i>
                                    @endif
                                    <div class="trading-link-copy__link--img">
                                        <img src="/images/info.png" alt="icon">
                                    </div>
                                </div>

                            </div>
                            <div class="trading-link__btn">
                                <button class="modal-bonus-btn red modal-accept active" type="submit">{{ __('Сохранить') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>


        <div class="servers-title">{{ __('Мой Инвентарь') }}</div>
        @include('pages.cabinet.inventory')

    </div>


    </div>
    </div>

    <div class="buying-item-modal balance-modal modal">
        <div class="modal-close"></div>

        <div class="modal-content buying-item">
            <div class="buying-item-close"><i class="fa-solid fa-xmark"></i></div>
            <div class="bi__content">
                <div class="bi__title">{{ __('Пополнение баланса') }}</div>

                <form action="{{ route('account.balance.topup') }}" method="POST" id="topup-balance-form">
                    @csrf
                    <input type="hidden" id="payment_id" name="payment_id" value="">

                    <div class="bi__items">

                        <div class="styles__MethodSelect-sc-ena6u4-2 jIjorv">
                            {{--
                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="47">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <svg viewBox="0 0 97 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M53.4313 28.7061H42.8158V9.59956H53.4313V28.7061Z" fill="#FF5F00"></path>
                                        <path d="M43.4958 19.1514C43.4958 15.2755 45.3078 11.823 48.1295 9.59807C46.066 7.97109 43.4618 7 40.6316 7C33.9313 7 28.5 12.4403 28.5 19.1514C28.5 25.8624 33.9313 31.3027 40.6316 31.3027C43.4618 31.3027 46.066 30.3316 48.1295 28.7046C45.3078 26.4797 43.4958 23.0272 43.4958 19.1514Z" fill="#EB001B"></path>
                                        <path d="M67.7486 19.1514C67.7486 25.8624 62.3172 31.3027 55.6169 31.3027C52.7867 31.3027 50.1825 30.3316 48.1183 28.7046C50.9408 26.4797 52.7528 23.0272 52.7528 19.1514C52.7528 15.2755 50.9408 11.823 48.1183 9.59807C50.1825 7.97109 52.7867 7 55.6169 7C62.3172 7 67.7486 12.4403 67.7486 19.1514Z" fill="#F79E1B"></path>
                                        <path d="M37.6287 19.2284C37.6287 18.6977 37.976 18.2616 38.5435 18.2616C39.0857 18.2616 39.4518 18.6789 39.4518 19.2284C39.4518 19.778 39.0857 20.1953 38.5435 20.1953C37.976 20.1953 37.6287 19.7592 37.6287 19.2284ZM40.0699 19.2284V17.7186H39.4143V18.0852C39.2063 17.8133 38.8908 17.6426 38.4619 17.6426C37.6164 17.6426 36.9536 18.3065 36.9536 19.2284C36.9536 20.1511 37.6164 20.8142 38.4619 20.8142C38.8908 20.8142 39.2063 20.6436 39.4143 20.3717V20.7383H40.0699V19.2284ZM62.2248 19.2284C62.2248 18.6977 62.5721 18.2616 63.1396 18.2616C63.6826 18.2616 64.0479 18.6789 64.0479 19.2284C64.0479 19.778 63.6826 20.1953 63.1396 20.1953C62.5721 20.1953 62.2248 19.7592 62.2248 19.2284ZM64.6667 19.2284V16.5059H64.0104V18.0852C63.8024 17.8133 63.4869 17.6426 63.058 17.6426C62.2125 17.6426 61.5497 18.3065 61.5497 19.2284C61.5497 20.1511 62.2125 20.8142 63.058 20.8142C63.4869 20.8142 63.8024 20.6436 64.0104 20.3717V20.7383H64.6667V19.2284ZM48.2079 18.2305C48.6303 18.2305 48.9018 18.4959 48.9711 18.9631H47.4065C47.4765 18.527 47.7408 18.2305 48.2079 18.2305ZM48.2209 17.6426C47.3371 17.6426 46.7191 18.2869 46.7191 19.2284C46.7191 20.1887 47.3624 20.8142 48.265 20.8142C48.7191 20.8142 49.135 20.7007 49.5011 20.3912L49.1798 19.9046C48.9271 20.1063 48.605 20.2206 48.3025 20.2206C47.8801 20.2206 47.4953 20.0246 47.4007 19.4808H49.6397C49.6462 19.3991 49.6527 19.3167 49.6527 19.2284C49.6462 18.2869 49.0657 17.6426 48.2209 17.6426ZM56.1373 19.2284C56.1373 18.6977 56.4846 18.2616 57.0521 18.2616C57.5944 18.2616 57.9604 18.6789 57.9604 19.2284C57.9604 19.778 57.5944 20.1953 57.0521 20.1953C56.4846 20.1953 56.1373 19.7592 56.1373 19.2284ZM58.5785 19.2284V17.7186H57.9229V18.0852C57.7142 17.8133 57.3994 17.6426 56.9705 17.6426C56.125 17.6426 55.4622 18.3065 55.4622 19.2284C55.4622 20.1511 56.125 20.8142 56.9705 20.8142C57.3994 20.8142 57.7142 20.6436 57.9229 20.3717V20.7383H58.5785V19.2284ZM52.4347 19.2284C52.4347 20.1446 53.0715 20.8142 54.0434 20.8142C54.4976 20.8142 54.8001 20.713 55.1279 20.4541L54.8131 19.9234C54.5669 20.1005 54.3084 20.1953 54.0239 20.1953C53.5004 20.1887 53.1156 19.8098 53.1156 19.2284C53.1156 18.6471 53.5004 18.2681 54.0239 18.2616C54.3084 18.2616 54.5669 18.3564 54.8131 18.5335L55.1279 18.0028C54.8001 17.7439 54.4976 17.6426 54.0434 17.6426C53.0715 17.6426 52.4347 18.3123 52.4347 19.2284ZM60.8876 17.6426C60.5092 17.6426 60.263 17.8198 60.0926 18.0852V17.7186H59.4428V20.7383H60.0991V19.0455C60.0991 18.5458 60.3135 18.2681 60.7424 18.2681C60.8753 18.2681 61.0139 18.2869 61.1525 18.3441L61.3547 17.7251C61.2096 17.668 61.0204 17.6426 60.8876 17.6426ZM43.3125 17.9587C42.997 17.7504 42.5623 17.6426 42.0829 17.6426C41.319 17.6426 40.8273 18.0093 40.8273 18.6095C40.8273 19.1019 41.1933 19.4056 41.8677 19.5003L42.1775 19.5444C42.5371 19.5951 42.7067 19.6898 42.7067 19.8604C42.7067 20.094 42.467 20.2271 42.0194 20.2271C41.5652 20.2271 41.2374 20.0817 41.0164 19.9111L40.7074 20.4223C41.067 20.6877 41.5212 20.8142 42.0129 20.8142C42.8836 20.8142 43.3883 20.4035 43.3883 19.8286C43.3883 19.2979 42.9912 19.0202 42.3349 18.9255L42.0259 18.8806C41.7421 18.843 41.5147 18.7866 41.5147 18.5841C41.5147 18.3629 41.7291 18.2305 42.0887 18.2305C42.4735 18.2305 42.8461 18.3759 43.0288 18.4894L43.3125 17.9587ZM51.7719 17.6426C51.3935 17.6426 51.1473 17.8198 50.9776 18.0852V17.7186H50.3278V20.7383H50.9834V19.0455C50.9834 18.5458 51.1979 18.2681 51.6267 18.2681C51.7596 18.2681 51.8982 18.2869 52.0369 18.3441L52.239 17.7251C52.0939 17.668 51.9047 17.6426 51.7719 17.6426ZM46.1768 17.7186H45.1046V16.8024H44.4418V17.7186H43.8302V18.3188H44.4418V19.6963C44.4418 20.397 44.7133 20.8142 45.4887 20.8142C45.7732 20.8142 46.101 20.726 46.309 20.5807L46.1198 20.0181C45.9241 20.1316 45.7097 20.1887 45.5393 20.1887C45.2115 20.1887 45.1046 19.9863 45.1046 19.6833V18.3188H46.1768V17.7186ZM36.3738 20.7383V18.843C36.3738 18.1293 35.9196 17.6492 35.1875 17.6426C34.8026 17.6361 34.4055 17.7562 34.1275 18.1799C33.9196 17.8451 33.5918 17.6426 33.1311 17.6426C32.8091 17.6426 32.4943 17.7374 32.2481 18.091V17.7186H31.5917V20.7383H32.2538V19.0643C32.2538 18.54 32.5441 18.2616 32.9925 18.2616C33.4279 18.2616 33.6481 18.5458 33.6481 19.0578V20.7383H34.3109V19.0643C34.3109 18.54 34.6134 18.2616 35.0488 18.2616C35.4965 18.2616 35.7109 18.5458 35.7109 19.0578V20.7383H36.3738Z" fill="white"></path>
                                        <path d="M45.2428 56.2244H40.8043L43.5805 40.2887H48.0187L45.2428 56.2244Z" fill="white"></path>
                                        <path d="M61.3322 40.6784C60.4567 40.356 59.0682 40 57.3513 40C52.9681 40 49.8815 42.1699 49.8626 45.2721C49.8262 47.5609 52.0725 48.8322 53.7526 49.5953C55.4697 50.3752 56.0535 50.8842 56.0535 51.5793C56.036 52.6469 54.6659 53.139 53.388 53.139C51.616 53.139 50.6665 52.8853 49.2236 52.2913L48.6391 52.0367L48.018 55.6139C49.059 56.0542 50.9769 56.4449 52.9681 56.462C57.6253 56.462 60.6572 54.3257 60.6932 51.0198C60.7109 49.2058 59.5247 47.8157 56.9675 46.6799C55.4151 45.9508 54.4644 45.4591 54.4644 44.7131C54.4826 44.0348 55.2685 43.3402 57.0209 43.3402C58.4638 43.3061 59.524 43.6281 60.3271 43.9503L60.7286 44.1196L61.3322 40.6784Z" fill="white"></path>
                                        <path d="M67.2316 50.579C67.5971 49.6635 69.0036 46.1203 69.0036 46.1203C68.9851 46.1543 69.3684 45.1879 69.5876 44.5946L69.8977 45.9677C69.8977 45.9677 70.7383 49.7822 70.9208 50.579C70.2271 50.579 68.1082 50.579 67.2316 50.579ZM72.7103 40.2887H69.2771C68.2184 40.2887 67.4141 40.5767 66.9573 41.6109L60.3645 56.2242H65.0217C65.0217 56.2242 65.7884 54.2573 65.9532 53.8337C66.4641 53.8337 70.9946 53.8337 71.6518 53.8337C71.7792 54.3932 72.1814 56.2242 72.1814 56.2242H76.291L72.7103 40.2887Z" fill="white"></path>
                                        <path d="M37.0972 40.2887L32.7505 51.1553L32.2755 48.9515C31.4718 46.4085 28.9515 43.6456 26.139 42.2718L30.1204 56.2075H34.814L41.7905 40.2887H37.0972Z" fill="white"></path>
                                        <path d="M28.7141 40.2887H21.5731L21.5 40.6107C27.0705 41.9331 30.7597 45.1208 32.2754 48.9521L30.7231 41.6283C30.4675 40.6105 29.6821 40.3223 28.7141 40.2887Z" fill="#FAA61A"></path>
                                    </svg>
                                </div>
                            </div>
                            --}}

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="47">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/viza_mc_rf.png" style="width: 99%;">
                                </div>
                            </div>

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="49">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 239 120">
                                        <path d="M206.665 34.254v29.338h-10.476v-20.58h-10.087v20.58h-10.476v-29.34h31.039z" style="fill: #fff"></path>
                                        <path d="M154.11 64.64c9.379 0 16.343-5.75 16.343-14.466 0-8.437-5.138-13.915-13.725-13.915-3.963 0-7.233 1.395-9.696 3.802.588-4.975 4.795-8.607 9.427-8.607 1.069 0 9.117-.017 9.117-.017l4.55-8.71s-10.103.23-14.8.23c-10.732.188-17.981 9.943-17.981 21.79 0 13.804 7.07 19.894 16.766 19.894zm.058-20.667c3.482 0 5.896 2.288 5.896 6.2 0 3.52-2.145 6.422-5.896 6.43-3.588 0-6.002-2.688-6.002-6.37 0-3.913 2.414-6.26 6.002-6.26z" style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"></path>
                                        <path d="M128.818 53.77s-2.474 1.426-6.17 1.696c-4.247.126-8.032-2.557-8.032-7.324 0-4.65 3.34-7.315 7.926-7.315 2.812 0 6.532 1.949 6.532 1.949s2.722-4.995 4.132-7.493c-2.582-1.957-6.021-3.03-10.021-3.03-10.095 0-17.914 6.582-17.914 15.83 0 9.366 7.349 15.795 17.914 15.6 2.953-.11 7.027-1.146 9.51-2.741z" style="fill: #fff"></path>
                                        <path fill="#5B57A2" d="m0 26.12 14.532 25.975v15.844L.017 93.863 0 26.12z"></path>
                                        <path fill="#D90751" d="m55.797 42.643 13.617-8.346 27.868-.026-41.485 25.414V42.643z"></path>
                                        <path fill="#FAB718" d="m55.72 25.967.077 34.39-14.566-8.95V0l14.49 25.967z"></path>
                                        <path fill="#ED6F26" d="m97.282 34.271-27.869.026-13.693-8.33L41.231 0l56.05 34.271z"></path>
                                        <path fill="#63B22F" d="M55.797 94.007V77.322l-14.566-8.78.008 51.458 14.558-25.993z"></path>
                                        <path fill="#1487C9" d="M69.38 85.737 14.531 52.095 0 26.12l97.223 59.583-27.844.034z"></path>
                                        <path fill="#017F36" d="m41.24 120 14.556-25.993 13.583-8.27 27.843-.034L41.24 120z"></path>
                                        <path fill="#984995" d="m.017 93.863 41.333-25.32-13.896-8.526-12.922 7.922L.017 93.863z"></path>
                                        <path d="M114.619 83.337c-.32.408-.741.716-1.246.924a4.282 4.282 0 0 1-1.632.316 4.63 4.63 0 0 1-1.633-.274 3.623 3.623 0 0 1-1.262-.782 3.539 3.539 0 0 1-.824-1.224 4.173 4.173 0 0 1-.295-1.59c0-.54.093-1.04.286-1.513a3.58 3.58 0 0 1 .79-1.224 3.88 3.88 0 0 1 1.196-.823c.463-.2.968-.308 1.523-.308.598 0 1.153.075 1.658.224.505.15.934.4 1.287.75l-.614 1.04a2.886 2.886 0 0 0-1.027-.6 3.612 3.612 0 0 0-1.12-.174c-.335 0-.655.066-.966.19a2.591 2.591 0 0 0-.825.534 2.438 2.438 0 0 0-.564.832 2.64 2.64 0 0 0-.21 1.08c0 .409.075.775.218 1.09.144.326.337.6.58.833.245.234.53.408.859.525a2.9 2.9 0 0 0 1.044.183c.454 0 .867-.092 1.22-.267a3.05 3.05 0 0 0 .926-.69zm1.733 1.065v-7.405h1.33v5.517l4.325-5.517h1.33v7.406h-1.33v-5.517l-4.325 5.517h-1.33zm15.795-1.065a3.01 3.01 0 0 1-1.245.924 4.291 4.291 0 0 1-1.633.316 4.628 4.628 0 0 1-1.632-.274 3.627 3.627 0 0 1-1.263-.782 3.539 3.539 0 0 1-.824-1.224 4.173 4.173 0 0 1-.295-1.59c0-.54.093-1.04.286-1.513.186-.475.455-.882.79-1.224a3.88 3.88 0 0 1 1.196-.823 3.637 3.637 0 0 1 1.523-.308c.598 0 1.153.075 1.658.224.505.15.934.4 1.288.75l-.615 1.04a2.874 2.874 0 0 0-1.026-.6 3.616 3.616 0 0 0-1.12-.174 2.603 2.603 0 0 0-1.792.724 2.536 2.536 0 0 0-.564.832 2.64 2.64 0 0 0-.21 1.08c0 .409.076.775.219 1.09.143.326.336.6.58.833.244.234.53.408.859.525.328.116.673.183 1.043.183.455 0 .859-.092 1.22-.267a3.05 3.05 0 0 0 .926-.69zm.682-6.34h6.824v1.315h-2.743v6.09h-1.33v-6.09h-2.743v-1.315z" style="fill: #fff"></path>
                                        <path d="M145.864 84.328c.505-.158.934-.4 1.304-.732l-.404-1.032c-.244.217-.572.4-.967.55-.396.15-.842.224-1.33.224-.732 0-1.338-.191-1.818-.582-.48-.392-.74-.94-.8-1.64h5.757a4 4 0 0 0 .084-.89 3.35 3.35 0 0 0-.278-1.381 3.277 3.277 0 0 0-.74-1.074 3.295 3.295 0 0 0-1.111-.69 3.692 3.692 0 0 0-1.355-.25c-.623 0-1.17.108-1.65.308a3.637 3.637 0 0 0-1.203.824 3.477 3.477 0 0 0-.748 1.223 4.272 4.272 0 0 0-.261 1.514c0 .583.1 1.115.286 1.59a3.5 3.5 0 0 0 .808 1.223c.345.34.765.599 1.262.782a4.772 4.772 0 0 0 1.658.274 5.04 5.04 0 0 0 1.506-.24zm-3.366-5.7c.412-.374.959-.557 1.649-.557.648 0 1.17.175 1.549.525.379.349.58.832.614 1.447h-4.544a2.26 2.26 0 0 1 .732-1.414z" style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"></path>
                                        <path d="M149.499 76.997h1.212l2.878 3.67 2.726-3.67h1.178v7.406h-1.33v-5.4l-2.591 3.436h-.051l-2.692-3.437v5.4h-1.33z" style="fill: #fff"></path>
                                        <path d="M161.11 77.022a4.187 4.187 0 0 0-1.143.49l.353 1.05c.303-.15.606-.275.909-.383.303-.11.69-.16 1.153-.16.37 0 .665.06.892.176.227.108.395.275.513.475.118.2.194.449.236.74.042.29.059.607.059.949a2.28 2.28 0 0 0-.91-.375 5.229 5.229 0 0 0-.958-.1c-.396 0-.766.059-1.12.158-.353.1-.656.25-.9.45a2.14 2.14 0 0 0-.59.74 2.3 2.3 0 0 0-.218 1.007c0 .724.21 1.29.63 1.69.422.399.977.598 1.667.598.623 0 1.12-.108 1.498-.324.379-.217.682-.466.9-.75v.958h1.246v-4.543c0-.966-.21-1.714-.64-2.24-.42-.523-1.17-.79-2.23-.79-.471 0-.917.067-1.346.184zm2.224 6c-.303.216-.7.315-1.187.315-.43 0-.766-.108-1.001-.316-.236-.216-.354-.5-.354-.85 0-.206.042-.381.135-.54a1.23 1.23 0 0 1 .362-.382 1.6 1.6 0 0 1 .513-.225c.194-.05.387-.075.589-.075.657 0 1.22.158 1.683.49v.94a3.824 3.824 0 0 1-.74.642zm13.733-8.63c.269-.124.52-.332.757-.64l-.724-1.04c-.185.25-.395.424-.631.516a4.007 4.007 0 0 1-.774.216l-.252.045c-.22.038-.454.08-.707.122a5.224 5.224 0 0 0-1.178.366c-.598.258-1.077.607-1.44 1.048-.361.44-.647.94-.841 1.49a7.793 7.793 0 0 0-.395 1.722c-.068.6-.101 1.173-.101 1.722 0 .708.092 1.348.277 1.922.186.575.446 1.057.79 1.448.346.4.759.7 1.247.916.488.216 1.035.324 1.649.324.564 0 1.086-.108 1.557-.308a3.758 3.758 0 0 0 1.212-.823c.336-.342.606-.75.79-1.207a3.74 3.74 0 0 0 .287-1.465c0-.557-.084-1.064-.244-1.514a3.271 3.271 0 0 0-.69-1.156 3.072 3.072 0 0 0-1.103-.75 3.827 3.827 0 0 0-1.447-.266c-.32 0-.631.042-.934.125a3.604 3.604 0 0 0-.842.358 3.2 3.2 0 0 0-.698.54c-.202.209-.37.442-.488.708h-.034a8.07 8.07 0 0 1 .143-1.048c.076-.358.185-.7.329-1.024.143-.324.328-.616.555-.882a2.52 2.52 0 0 1 .875-.624 4.646 4.646 0 0 1 1.153-.374l.264-.048c.263-.048.514-.094.754-.144.32-.066.614-.15.884-.274zm-4.68 7.115a4.146 4.146 0 0 1-.184-1.298 2.09 2.09 0 0 1 .303-.707 2.48 2.48 0 0 1 .547-.6 2.773 2.773 0 0 1 1.733-.59c.783 0 1.372.24 1.784.716.413.482.615 1.08.615 1.805 0 .358-.068.683-.185.982a2.36 2.36 0 0 1-1.28 1.323 2.45 2.45 0 0 1-1.001.2c-.37 0-.707-.075-1.018-.225a2.34 2.34 0 0 1-.8-.624 3.129 3.129 0 0 1-.513-.982zm16.402-4.51h-1.33v7.406h1.33zm-8.281 0h1.33v2.222h1.229c.58 0 1.05.075 1.43.216.379.142.682.333.909.566.227.233.387.508.48.815.092.308.143.624.143.957 0 .333-.051.65-.16.966-.101.316-.278.59-.514.84-.235.25-.555.45-.96.6-.403.149-.891.232-1.48.232H180.5v-7.414zm1.33 3.46v2.714h.968c.665 0 1.136-.116 1.405-.35.27-.233.404-.565.404-1.006 0-.45-.143-.79-.412-1.024-.278-.233-.741-.35-1.38-.35h-.985z" style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"></path>
                                        <path d="M197.583 83.337c-.32.408-.741.716-1.246.924a4.286 4.286 0 0 1-1.632.316 4.63 4.63 0 0 1-1.633-.274 3.613 3.613 0 0 1-1.262-.782 3.555 3.555 0 0 1-.825-1.224 4.192 4.192 0 0 1-.294-1.59c0-.54.092-1.04.286-1.513a3.58 3.58 0 0 1 .79-1.224c.338-.35.74-.615 1.196-.823a3.634 3.634 0 0 1 1.523-.308c.597 0 1.153.075 1.658.224.505.15.934.4 1.287.75l-.614 1.04a2.886 2.886 0 0 0-1.027-.6 3.612 3.612 0 0 0-1.12-.174c-.335 0-.655.066-.967.19a2.587 2.587 0 0 0-.824.534 2.52 2.52 0 0 0-.564.832 2.64 2.64 0 0 0-.21 1.08c0 .409.075.775.218 1.09.143.326.337.6.58.833.245.234.53.408.859.525.329.116.674.183 1.044.183a2.76 2.76 0 0 0 1.22-.267 3.05 3.05 0 0 0 .926-.69zm.68-6.34h6.826v1.315h-2.744v6.09h-1.33v-6.09h-2.742v-1.315z" style="fill: #fff"></path>
                                        <path d="M207.866 76.998h-1.33v10.7h1.33v-3.636c.269.175.589.3.959.383.37.083.757.124 1.153.124.589 0 1.119-.108 1.599-.307a3.825 3.825 0 0 0 1.228-.84 3.65 3.65 0 0 0 .79-1.266c.186-.49.279-1.015.279-1.572 0-.55-.084-1.057-.244-1.515a3.524 3.524 0 0 0-.69-1.181 2.996 2.996 0 0 0-1.086-.774 3.49 3.49 0 0 0-1.44-.283c-.52 0-1.008.1-1.471.3-.463.2-.825.44-1.077.74zm.926 1.373c.379-.2.79-.3 1.237-.3.387 0 .732.058 1.044.183.303.125.555.3.766.533.21.233.37.5.47.815.11.316.16.658.16 1.032 0 .4-.067.757-.184 1.09a2.4 2.4 0 0 1-.514.849 2.36 2.36 0 0 1-.816.557 2.685 2.685 0 0 1-1.086.208c-.361 0-.698-.041-1.018-.124a3.328 3.328 0 0 1-.984-.45v-3.67c.235-.282.547-.523.925-.723zm15.28-1.374h-1.33v7.406h1.33zm-8.288 0h1.33v2.222h1.228c.58 0 1.052.075 1.43.216.38.142.683.333.91.566.227.233.387.508.479.815.093.308.143.624.143.957 0 .333-.05.65-.16.966-.1.316-.277.59-.513.84-.235.25-.555.45-.96.6-.403.149-.891.232-1.48.232h-2.415v-7.414zm1.339 3.46v2.714h.968c.664 0 1.136-.116 1.405-.35.269-.233.404-.565.404-1.006 0-.45-.143-.79-.412-1.024-.278-.233-.741-.35-1.38-.35h-.985z" style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"></path>
                                        <path d="m230.452 80.567 2.844 3.836h-1.632l-2.197-2.996-2.23 2.996h-1.548l2.827-3.77-2.66-3.636h1.634l2.028 2.796 2.06-2.796h1.55zM113.635 93.556h-4.081v6.174h-1.33v-7.405h6.74v7.405h-1.33zm8.877.083h-2.625l-.085 1.248c-.092 1.057-.218 1.906-.395 2.554-.177.65-.396 1.149-.648 1.498-.252.35-.547.59-.884.708a3.06 3.06 0 0 1-1.085.183l-.101-1.282a1.08 1.08 0 0 0 .496-.108c.186-.083.37-.266.547-.54.177-.284.337-.683.48-1.2.143-.523.236-1.205.286-2.063l.135-2.305h5.209v7.406h-1.33v-6.1z" style="fill: #fff"></path>
                                        <path d="M127.427 92.342c-.43.116-.817.282-1.145.49l.354 1.049c.303-.15.606-.275.908-.383.303-.108.69-.158 1.153-.158.37 0 .665.058.892.174.228.109.396.275.514.475.117.2.193.45.235.74.042.292.059.608.059.949a2.277 2.277 0 0 0-.91-.374 5.22 5.22 0 0 0-.958-.1c-.395 0-.766.058-1.12.158-.353.1-.656.25-.9.45a2.14 2.14 0 0 0-.59.74 2.3 2.3 0 0 0-.217 1.007c0 .723.21 1.29.63 1.689.42.399.977.599 1.667.599.623 0 1.119-.108 1.498-.325.378-.216.68-.466.9-.75v.958h1.246v-4.543c0-.965-.211-1.714-.64-2.238-.421-.525-1.17-.79-2.23-.79a5.1 5.1 0 0 0-1.346.182zm2.23 5.998c-.302.217-.698.317-1.186.317-.438 0-.766-.108-1.001-.317-.236-.216-.354-.5-.354-.848 0-.208.042-.383.135-.541.092-.158.219-.283.362-.383.143-.1.319-.175.513-.225a2.33 2.33 0 0 1 .589-.074c.656 0 1.22.158 1.683.49v.94a3.83 3.83 0 0 1-.741.64z" style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"></path>
                                        <path d="M132.946 92.325h6.825v1.314h-2.744v6.09h-1.33v-6.09h-2.742v-1.314z" style="fill: #fff"></path>
                                        <path d="M145.982 99.655c.505-.158.934-.4 1.304-.732l-.404-1.032c-.244.217-.572.4-.968.55-.395.15-.841.224-1.33.224-.731 0-1.337-.191-1.817-.582-.48-.391-.74-.94-.8-1.64h5.757c.058-.257.084-.55.084-.89 0-.507-.093-.965-.278-1.381a3.264 3.264 0 0 0-.741-1.073 3.28 3.28 0 0 0-1.11-.691 3.696 3.696 0 0 0-1.355-.25c-.623 0-1.17.108-1.65.308a3.618 3.618 0 0 0-1.203.824 3.494 3.494 0 0 0-.75 1.223 4.294 4.294 0 0 0-.26 1.514c0 .583.1 1.115.286 1.59.186.474.463.882.808 1.223a3.47 3.47 0 0 0 1.263.782 4.766 4.766 0 0 0 1.657.275c.497 0 1.002-.084 1.507-.242zm-3.374-5.708c.412-.374.959-.557 1.649-.557.657 0 1.17.183 1.549.524.378.35.58.832.614 1.448h-4.544a2.26 2.26 0 0 1 .732-1.415zm5.848-1.622h1.683l2.566 3.57-2.743 3.835h-1.632l2.877-3.836-2.751-3.57zm4.577 0h1.33v7.405h-1.33zm6.084 7.405-2.928-3.836 2.735-3.57h-1.65l-2.583 3.57 2.777 3.836zm6.076-.075c.505-.158.934-.4 1.304-.732l-.404-1.032c-.244.217-.572.4-.967.55-.396.15-.842.224-1.33.224-.732 0-1.338-.191-1.818-.582-.48-.391-.74-.94-.8-1.64h5.757c.059-.257.084-.55.084-.89 0-.507-.092-.965-.278-1.381a3.276 3.276 0 0 0-.74-1.073 3.284 3.284 0 0 0-1.111-.691 3.692 3.692 0 0 0-1.355-.25c-.622 0-1.17.108-1.65.308a3.624 3.624 0 0 0-1.202.824 3.465 3.465 0 0 0-.75 1.223 4.272 4.272 0 0 0-.26 1.514c0 .583.1 1.115.286 1.59.185.474.463.882.808 1.223.345.34.765.599 1.262.782a4.772 4.772 0 0 0 1.658.275 5.04 5.04 0 0 0 1.506-.242zm-3.374-5.708c.413-.374.96-.557 1.65-.557.648 0 1.169.183 1.548.524.379.35.58.832.614 1.448h-4.544a2.26 2.26 0 0 1 .732-1.415zm10.595-4.11c-.783 0-1.212-.433-1.28-1.298h-1.253c0 .332.059.632.168.915.11.283.27.524.48.724.21.2.48.358.79.482.312.125.674.183 1.086.183a2.64 2.64 0 0 0 1.019-.183c.303-.116.555-.282.774-.482a2.1 2.1 0 0 0 .496-.724c.118-.283.177-.583.177-.915h-1.245c-.06.865-.472 1.298-1.212 1.298zm-3.594 2.487v7.406h1.33l4.325-5.517v5.517h1.33v-7.406h-1.33l-4.325 5.517v-5.517z" style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"></path>
                                    </svg>
                                </div>
                            </div>

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d tebex" data-payment_id="48">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/viza_mc_world.png" style="width: 99%;">
                                </div>
                            </div>

                            {{--
                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d unitpay" data-payment_id="451">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/viza_mc_world2.png" style="width: 99%;">
                                </div>
                            </div>
                            --}}

                            <!-- <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="21">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/PayPal.png">
                                </div>
                            </div> -->
                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="48">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/PayPal.png">
                                </div>
                            </div>

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d tebex" data-payment_id="48">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/Alipay.png" alt="mir logo" style="width: 75%;">
                                </div>
                            </div>

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d tebex" data-payment_id="48">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/GPAY.png" alt="mir logo" style="width: 75%;">
                                </div>
                            </div>

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d tebex" data-payment_id="48">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/JCB_logo.svg.png" alt="mir logo" style="width: 45%;">
                                </div>
                            </div>

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d tebex" data-payment_id="48">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/AmericanExpress_.png" alt="mir logo" style="width: 52%;">
                                </div>
                            </div>


                            <!-- <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d alfa" data-payment_id="47">
                    <div
                            class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM"
                            fill="inherit"
                    >
                      <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 239 120"
                      >
                        <path
                                d="M206.665 34.254v29.338h-10.476v-20.58h-10.087v20.58h-10.476v-29.34h31.039z"
                                style="fill: #fff"
                        ></path>
                        <path
                                d="M154.11 64.64c9.379 0 16.343-5.75 16.343-14.466 0-8.437-5.138-13.915-13.725-13.915-3.963 0-7.233 1.395-9.696 3.802.588-4.975 4.795-8.607 9.427-8.607 1.069 0 9.117-.017 9.117-.017l4.55-8.71s-10.103.23-14.8.23c-10.732.188-17.981 9.943-17.981 21.79 0 13.804 7.07 19.894 16.766 19.894zm.058-20.667c3.482 0 5.896 2.288 5.896 6.2 0 3.52-2.145 6.422-5.896 6.43-3.588 0-6.002-2.688-6.002-6.37 0-3.913 2.414-6.26 6.002-6.26z"
                                style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"
                        ></path>
                        <path
                                d="M128.818 53.77s-2.474 1.426-6.17 1.696c-4.247.126-8.032-2.557-8.032-7.324 0-4.65 3.34-7.315 7.926-7.315 2.812 0 6.532 1.949 6.532 1.949s2.722-4.995 4.132-7.493c-2.582-1.957-6.021-3.03-10.021-3.03-10.095 0-17.914 6.582-17.914 15.83 0 9.366 7.349 15.795 17.914 15.6 2.953-.11 7.027-1.146 9.51-2.741z"
                                style="fill: #fff"
                        ></path>
                        <path
                                fill="#5B57A2"
                                d="m0 26.12 14.532 25.975v15.844L.017 93.863 0 26.12z"
                        ></path>
                        <path
                                fill="#D90751"
                                d="m55.797 42.643 13.617-8.346 27.868-.026-41.485 25.414V42.643z"
                        ></path>
                        <path
                                fill="#FAB718"
                                d="m55.72 25.967.077 34.39-14.566-8.95V0l14.49 25.967z"
                        ></path>
                        <path
                                fill="#ED6F26"
                                d="m97.282 34.271-27.869.026-13.693-8.33L41.231 0l56.05 34.271z"
                        ></path>
                        <path
                                fill="#63B22F"
                                d="M55.797 94.007V77.322l-14.566-8.78.008 51.458 14.558-25.993z"
                        ></path>
                        <path
                                fill="#1487C9"
                                d="M69.38 85.737 14.531 52.095 0 26.12l97.223 59.583-27.844.034z"
                        ></path>
                        <path
                                fill="#017F36"
                                d="m41.24 120 14.556-25.993 13.583-8.27 27.843-.034L41.24 120z"
                        ></path>
                        <path
                                fill="#984995"
                                d="m.017 93.863 41.333-25.32-13.896-8.526-12.922 7.922L.017 93.863z"
                        ></path>
                        <path
                                d="M114.619 83.337c-.32.408-.741.716-1.246.924a4.282 4.282 0 0 1-1.632.316 4.63 4.63 0 0 1-1.633-.274 3.623 3.623 0 0 1-1.262-.782 3.539 3.539 0 0 1-.824-1.224 4.173 4.173 0 0 1-.295-1.59c0-.54.093-1.04.286-1.513a3.58 3.58 0 0 1 .79-1.224 3.88 3.88 0 0 1 1.196-.823c.463-.2.968-.308 1.523-.308.598 0 1.153.075 1.658.224.505.15.934.4 1.287.75l-.614 1.04a2.886 2.886 0 0 0-1.027-.6 3.612 3.612 0 0 0-1.12-.174c-.335 0-.655.066-.966.19a2.591 2.591 0 0 0-.825.534 2.438 2.438 0 0 0-.564.832 2.64 2.64 0 0 0-.21 1.08c0 .409.075.775.218 1.09.144.326.337.6.58.833.245.234.53.408.859.525a2.9 2.9 0 0 0 1.044.183c.454 0 .867-.092 1.22-.267a3.05 3.05 0 0 0 .926-.69zm1.733 1.065v-7.405h1.33v5.517l4.325-5.517h1.33v7.406h-1.33v-5.517l-4.325 5.517h-1.33zm15.795-1.065a3.01 3.01 0 0 1-1.245.924 4.291 4.291 0 0 1-1.633.316 4.628 4.628 0 0 1-1.632-.274 3.627 3.627 0 0 1-1.263-.782 3.539 3.539 0 0 1-.824-1.224 4.173 4.173 0 0 1-.295-1.59c0-.54.093-1.04.286-1.513.186-.475.455-.882.79-1.224a3.88 3.88 0 0 1 1.196-.823 3.637 3.637 0 0 1 1.523-.308c.598 0 1.153.075 1.658.224.505.15.934.4 1.288.75l-.615 1.04a2.874 2.874 0 0 0-1.026-.6 3.616 3.616 0 0 0-1.12-.174 2.603 2.603 0 0 0-1.792.724 2.536 2.536 0 0 0-.564.832 2.64 2.64 0 0 0-.21 1.08c0 .409.076.775.219 1.09.143.326.336.6.58.833.244.234.53.408.859.525.328.116.673.183 1.043.183.455 0 .859-.092 1.22-.267a3.05 3.05 0 0 0 .926-.69zm.682-6.34h6.824v1.315h-2.743v6.09h-1.33v-6.09h-2.743v-1.315z"
                                style="fill: #fff"
                        ></path>
                        <path
                                d="M145.864 84.328c.505-.158.934-.4 1.304-.732l-.404-1.032c-.244.217-.572.4-.967.55-.396.15-.842.224-1.33.224-.732 0-1.338-.191-1.818-.582-.48-.392-.74-.94-.8-1.64h5.757a4 4 0 0 0 .084-.89 3.35 3.35 0 0 0-.278-1.381 3.277 3.277 0 0 0-.74-1.074 3.295 3.295 0 0 0-1.111-.69 3.692 3.692 0 0 0-1.355-.25c-.623 0-1.17.108-1.65.308a3.637 3.637 0 0 0-1.203.824 3.477 3.477 0 0 0-.748 1.223 4.272 4.272 0 0 0-.261 1.514c0 .583.1 1.115.286 1.59a3.5 3.5 0 0 0 .808 1.223c.345.34.765.599 1.262.782a4.772 4.772 0 0 0 1.658.274 5.04 5.04 0 0 0 1.506-.24zm-3.366-5.7c.412-.374.959-.557 1.649-.557.648 0 1.17.175 1.549.525.379.349.58.832.614 1.447h-4.544a2.26 2.26 0 0 1 .732-1.414z"
                                style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"
                        ></path>
                        <path
                                d="M149.499 76.997h1.212l2.878 3.67 2.726-3.67h1.178v7.406h-1.33v-5.4l-2.591 3.436h-.051l-2.692-3.437v5.4h-1.33z"
                                style="fill: #fff"
                        ></path>
                        <path
                                d="M161.11 77.022a4.187 4.187 0 0 0-1.143.49l.353 1.05c.303-.15.606-.275.909-.383.303-.11.69-.16 1.153-.16.37 0 .665.06.892.176.227.108.395.275.513.475.118.2.194.449.236.74.042.29.059.607.059.949a2.28 2.28 0 0 0-.91-.375 5.229 5.229 0 0 0-.958-.1c-.396 0-.766.059-1.12.158-.353.1-.656.25-.9.45a2.14 2.14 0 0 0-.59.74 2.3 2.3 0 0 0-.218 1.007c0 .724.21 1.29.63 1.69.422.399.977.598 1.667.598.623 0 1.12-.108 1.498-.324.379-.217.682-.466.9-.75v.958h1.246v-4.543c0-.966-.21-1.714-.64-2.24-.42-.523-1.17-.79-2.23-.79-.471 0-.917.067-1.346.184zm2.224 6c-.303.216-.7.315-1.187.315-.43 0-.766-.108-1.001-.316-.236-.216-.354-.5-.354-.85 0-.206.042-.381.135-.54a1.23 1.23 0 0 1 .362-.382 1.6 1.6 0 0 1 .513-.225c.194-.05.387-.075.589-.075.657 0 1.22.158 1.683.49v.94a3.824 3.824 0 0 1-.74.642zm13.733-8.63c.269-.124.52-.332.757-.64l-.724-1.04c-.185.25-.395.424-.631.516a4.007 4.007 0 0 1-.774.216l-.252.045c-.22.038-.454.08-.707.122a5.224 5.224 0 0 0-1.178.366c-.598.258-1.077.607-1.44 1.048-.361.44-.647.94-.841 1.49a7.793 7.793 0 0 0-.395 1.722c-.068.6-.101 1.173-.101 1.722 0 .708.092 1.348.277 1.922.186.575.446 1.057.79 1.448.346.4.759.7 1.247.916.488.216 1.035.324 1.649.324.564 0 1.086-.108 1.557-.308a3.758 3.758 0 0 0 1.212-.823c.336-.342.606-.75.79-1.207a3.74 3.74 0 0 0 .287-1.465c0-.557-.084-1.064-.244-1.514a3.271 3.271 0 0 0-.69-1.156 3.072 3.072 0 0 0-1.103-.75 3.827 3.827 0 0 0-1.447-.266c-.32 0-.631.042-.934.125a3.604 3.604 0 0 0-.842.358 3.2 3.2 0 0 0-.698.54c-.202.209-.37.442-.488.708h-.034a8.07 8.07 0 0 1 .143-1.048c.076-.358.185-.7.329-1.024.143-.324.328-.616.555-.882a2.52 2.52 0 0 1 .875-.624 4.646 4.646 0 0 1 1.153-.374l.264-.048c.263-.048.514-.094.754-.144.32-.066.614-.15.884-.274zm-4.68 7.115a4.146 4.146 0 0 1-.184-1.298 2.09 2.09 0 0 1 .303-.707 2.48 2.48 0 0 1 .547-.6 2.773 2.773 0 0 1 1.733-.59c.783 0 1.372.24 1.784.716.413.482.615 1.08.615 1.805 0 .358-.068.683-.185.982a2.36 2.36 0 0 1-1.28 1.323 2.45 2.45 0 0 1-1.001.2c-.37 0-.707-.075-1.018-.225a2.34 2.34 0 0 1-.8-.624 3.129 3.129 0 0 1-.513-.982zm16.402-4.51h-1.33v7.406h1.33zm-8.281 0h1.33v2.222h1.229c.58 0 1.05.075 1.43.216.379.142.682.333.909.566.227.233.387.508.48.815.092.308.143.624.143.957 0 .333-.051.65-.16.966-.101.316-.278.59-.514.84-.235.25-.555.45-.96.6-.403.149-.891.232-1.48.232H180.5v-7.414zm1.33 3.46v2.714h.968c.665 0 1.136-.116 1.405-.35.27-.233.404-.565.404-1.006 0-.45-.143-.79-.412-1.024-.278-.233-.741-.35-1.38-.35h-.985z"
                                style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"
                        ></path>
                        <path
                                d="M197.583 83.337c-.32.408-.741.716-1.246.924a4.286 4.286 0 0 1-1.632.316 4.63 4.63 0 0 1-1.633-.274 3.613 3.613 0 0 1-1.262-.782 3.555 3.555 0 0 1-.825-1.224 4.192 4.192 0 0 1-.294-1.59c0-.54.092-1.04.286-1.513a3.58 3.58 0 0 1 .79-1.224c.338-.35.74-.615 1.196-.823a3.634 3.634 0 0 1 1.523-.308c.597 0 1.153.075 1.658.224.505.15.934.4 1.287.75l-.614 1.04a2.886 2.886 0 0 0-1.027-.6 3.612 3.612 0 0 0-1.12-.174c-.335 0-.655.066-.967.19a2.587 2.587 0 0 0-.824.534 2.52 2.52 0 0 0-.564.832 2.64 2.64 0 0 0-.21 1.08c0 .409.075.775.218 1.09.143.326.337.6.58.833.245.234.53.408.859.525.329.116.674.183 1.044.183a2.76 2.76 0 0 0 1.22-.267 3.05 3.05 0 0 0 .926-.69zm.68-6.34h6.826v1.315h-2.744v6.09h-1.33v-6.09h-2.742v-1.315z"
                                style="fill: #fff"
                        ></path>
                        <path
                                d="M207.866 76.998h-1.33v10.7h1.33v-3.636c.269.175.589.3.959.383.37.083.757.124 1.153.124.589 0 1.119-.108 1.599-.307a3.825 3.825 0 0 0 1.228-.84 3.65 3.65 0 0 0 .79-1.266c.186-.49.279-1.015.279-1.572 0-.55-.084-1.057-.244-1.515a3.524 3.524 0 0 0-.69-1.181 2.996 2.996 0 0 0-1.086-.774 3.49 3.49 0 0 0-1.44-.283c-.52 0-1.008.1-1.471.3-.463.2-.825.44-1.077.74zm.926 1.373c.379-.2.79-.3 1.237-.3.387 0 .732.058 1.044.183.303.125.555.3.766.533.21.233.37.5.47.815.11.316.16.658.16 1.032 0 .4-.067.757-.184 1.09a2.4 2.4 0 0 1-.514.849 2.36 2.36 0 0 1-.816.557 2.685 2.685 0 0 1-1.086.208c-.361 0-.698-.041-1.018-.124a3.328 3.328 0 0 1-.984-.45v-3.67c.235-.282.547-.523.925-.723zm15.28-1.374h-1.33v7.406h1.33zm-8.288 0h1.33v2.222h1.228c.58 0 1.052.075 1.43.216.38.142.683.333.91.566.227.233.387.508.479.815.093.308.143.624.143.957 0 .333-.05.65-.16.966-.1.316-.277.59-.513.84-.235.25-.555.45-.96.6-.403.149-.891.232-1.48.232h-2.415v-7.414zm1.339 3.46v2.714h.968c.664 0 1.136-.116 1.405-.35.269-.233.404-.565.404-1.006 0-.45-.143-.79-.412-1.024-.278-.233-.741-.35-1.38-.35h-.985z"
                                style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"
                        ></path>
                        <path
                                d="m230.452 80.567 2.844 3.836h-1.632l-2.197-2.996-2.23 2.996h-1.548l2.827-3.77-2.66-3.636h1.634l2.028 2.796 2.06-2.796h1.55zM113.635 93.556h-4.081v6.174h-1.33v-7.405h6.74v7.405h-1.33zm8.877.083h-2.625l-.085 1.248c-.092 1.057-.218 1.906-.395 2.554-.177.65-.396 1.149-.648 1.498-.252.35-.547.59-.884.708a3.06 3.06 0 0 1-1.085.183l-.101-1.282a1.08 1.08 0 0 0 .496-.108c.186-.083.37-.266.547-.54.177-.284.337-.683.48-1.2.143-.523.236-1.205.286-2.063l.135-2.305h5.209v7.406h-1.33v-6.1z"
                                style="fill: #fff"
                        ></path>
                        <path
                                d="M127.427 92.342c-.43.116-.817.282-1.145.49l.354 1.049c.303-.15.606-.275.908-.383.303-.108.69-.158 1.153-.158.37 0 .665.058.892.174.228.109.396.275.514.475.117.2.193.45.235.74.042.292.059.608.059.949a2.277 2.277 0 0 0-.91-.374 5.22 5.22 0 0 0-.958-.1c-.395 0-.766.058-1.12.158-.353.1-.656.25-.9.45a2.14 2.14 0 0 0-.59.74 2.3 2.3 0 0 0-.217 1.007c0 .723.21 1.29.63 1.689.42.399.977.599 1.667.599.623 0 1.119-.108 1.498-.325.378-.216.68-.466.9-.75v.958h1.246v-4.543c0-.965-.211-1.714-.64-2.238-.421-.525-1.17-.79-2.23-.79a5.1 5.1 0 0 0-1.346.182zm2.23 5.998c-.302.217-.698.317-1.186.317-.438 0-.766-.108-1.001-.317-.236-.216-.354-.5-.354-.848 0-.208.042-.383.135-.541.092-.158.219-.283.362-.383.143-.1.319-.175.513-.225a2.33 2.33 0 0 1 .589-.074c.656 0 1.22.158 1.683.49v.94a3.83 3.83 0 0 1-.741.64z"
                                style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"
                        ></path>
                        <path
                                d="M132.946 92.325h6.825v1.314h-2.744v6.09h-1.33v-6.09h-2.742v-1.314z"
                                style="fill: #fff"
                        ></path>
                        <path
                                d="M145.982 99.655c.505-.158.934-.4 1.304-.732l-.404-1.032c-.244.217-.572.4-.968.55-.395.15-.841.224-1.33.224-.731 0-1.337-.191-1.817-.582-.48-.391-.74-.94-.8-1.64h5.757c.058-.257.084-.55.084-.89 0-.507-.093-.965-.278-1.381a3.264 3.264 0 0 0-.741-1.073 3.28 3.28 0 0 0-1.11-.691 3.696 3.696 0 0 0-1.355-.25c-.623 0-1.17.108-1.65.308a3.618 3.618 0 0 0-1.203.824 3.494 3.494 0 0 0-.75 1.223 4.294 4.294 0 0 0-.26 1.514c0 .583.1 1.115.286 1.59.186.474.463.882.808 1.223a3.47 3.47 0 0 0 1.263.782 4.766 4.766 0 0 0 1.657.275c.497 0 1.002-.084 1.507-.242zm-3.374-5.708c.412-.374.959-.557 1.649-.557.657 0 1.17.183 1.549.524.378.35.58.832.614 1.448h-4.544a2.26 2.26 0 0 1 .732-1.415zm5.848-1.622h1.683l2.566 3.57-2.743 3.835h-1.632l2.877-3.836-2.751-3.57zm4.577 0h1.33v7.405h-1.33zm6.084 7.405-2.928-3.836 2.735-3.57h-1.65l-2.583 3.57 2.777 3.836zm6.076-.075c.505-.158.934-.4 1.304-.732l-.404-1.032c-.244.217-.572.4-.967.55-.396.15-.842.224-1.33.224-.732 0-1.338-.191-1.818-.582-.48-.391-.74-.94-.8-1.64h5.757c.059-.257.084-.55.084-.89 0-.507-.092-.965-.278-1.381a3.276 3.276 0 0 0-.74-1.073 3.284 3.284 0 0 0-1.111-.691 3.692 3.692 0 0 0-1.355-.25c-.622 0-1.17.108-1.65.308a3.624 3.624 0 0 0-1.202.824 3.465 3.465 0 0 0-.75 1.223 4.272 4.272 0 0 0-.26 1.514c0 .583.1 1.115.286 1.59.185.474.463.882.808 1.223.345.34.765.599 1.262.782a4.772 4.772 0 0 0 1.658.275 5.04 5.04 0 0 0 1.506-.242zm-3.374-5.708c.413-.374.96-.557 1.65-.557.648 0 1.169.183 1.548.524.379.35.58.832.614 1.448h-4.544a2.26 2.26 0 0 1 .732-1.415zm10.595-4.11c-.783 0-1.212-.433-1.28-1.298h-1.253c0 .332.059.632.168.915.11.283.27.524.48.724.21.2.48.358.79.482.312.125.674.183 1.086.183a2.64 2.64 0 0 0 1.019-.183c.303-.116.555-.282.774-.482a2.1 2.1 0 0 0 .496-.724c.118-.283.177-.583.177-.915h-1.245c-.06.865-.472 1.298-1.212 1.298zm-3.594 2.487v7.406h1.33l4.325-5.517v5.517h1.33v-7.406h-1.33l-4.325 5.517v-5.517z"
                                style="clip-rule: evenodd; fill: #fff; fill-rule: evenodd"
                        ></path>
                      </svg>
                    </div>
                  </div> -->

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d steam" data-payment_id="5">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/steam-logo3.png" alt="steam logo" style="width: 75%;">
                                </div>
                            </div>

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="51">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 306.5 64.001">
                                        <path fill="#f7931a" d="M63.033 39.745C58.759 56.888 41.396 67.321 24.25 63.046 7.113 58.772-3.32 41.408.956 24.266 5.228 7.121 22.59-3.313 39.73.961c17.144 4.274 27.576 21.64 23.302 38.784z"></path>
                                        <path fill="#FFF" d="M46.103 27.445c.637-4.258-2.605-6.547-7.038-8.074l1.438-5.768-3.511-.875-1.4 5.616c-.923-.23-1.871-.447-2.813-.662l1.41-5.653-3.51-.875-1.438 5.766c-.764-.174-1.514-.346-2.242-.527l.004-.018-4.842-1.209-.934 3.75s2.605.597 2.55.634c1.422.355 1.679 1.296 1.636 2.042l-1.638 6.571c.098.025.225.061.365.117l-.371-.092-2.296 9.205c-.174.432-.615 1.08-1.61.834.036.051-2.551-.637-2.551-.637l-1.743 4.02 4.569 1.138c.85.213 1.683.436 2.503.646l-1.453 5.834 3.507.875 1.439-5.772c.958.26 1.888.5 2.798.726l-1.434 5.745 3.51.875 1.454-5.823c5.987 1.133 10.489.676 12.384-4.739 1.527-4.36-.076-6.875-3.226-8.515 2.294-.529 4.022-2.038 4.483-5.155zm-8.022 11.25c-1.085 4.36-8.426 2.002-10.806 1.411l1.928-7.729c2.38.594 10.012 1.77 8.878 6.317zm1.086-11.313c-.99 3.966-7.1 1.951-9.082 1.457l1.748-7.01c1.982.494 8.365 1.416 7.334 5.553z"></path>
                                        <path fill="#ffffff" d="M93.773 19.365c2.595 0 4.837.465 6.72 1.378 1.894.922 3.456 2.164 4.709 3.726 1.236 1.57 2.156 3.405 2.75 5.508.59 2.11.886 4.376.886 6.803 0 3.728-.683 7.25-2.062 10.57-1.38 3.325-3.25 6.21-5.63 8.67-2.378 2.456-5.186 4.393-8.424 5.824-3.233 1.432-6.748 2.148-10.522 2.148-.488 0-1.346-.014-2.558-.039s-2.605-.15-4.165-.36a48.879 48.879 0 0 1-4.983-.978 24.966 24.966 0 0 1-4.983-1.78L79.523 1.957 92.073.013l-5.017 20.893a19.51 19.51 0 0 1 3.236-1.132 14.375 14.375 0 0 1 3.48-.409zM83.246 54.036c1.89 0 3.67-.465 5.344-1.378a14.067 14.067 0 0 0 4.339-3.685c1.213-1.544 2.173-3.283 2.873-5.226s1.054-3.97 1.054-6.079c0-2.59-.433-4.612-1.296-6.073-.863-1.455-2.46-2.187-4.78-2.187-.76 0-1.738.145-2.952.404-1.218.275-2.308.846-3.285 1.705L79.2 53.705c.322.057.607.111.85.162.238.055.5.094.763.121.277.031.594.047.977.047l1.455.001zM121.853 62.865h-11.987l10.123-42.597h12.069l-10.205 42.597zm5.833-47.787a7.438 7.438 0 0 1-4.536-1.496c-1.357-.992-2.03-2.519-2.03-4.577 0-1.132.23-2.194.687-3.196a8.55 8.55 0 0 1 1.826-2.593 8.967 8.967 0 0 1 2.63-1.743 8.031 8.031 0 0 1 3.204-.645c1.672 0 3.18.498 4.532 1.496 1.346 1.003 2.023 2.53 2.023 4.577a7.565 7.565 0 0 1-.69 3.202 8.43 8.43 0 0 1-1.82 2.593 8.805 8.805 0 0 1-2.63 1.738c-1.002.437-2.064.644-3.196.644zM142.563 9.655l12.555-1.945-3.083 12.556h13.446l-2.428 9.878h-13.365l-3.56 14.9c-.328 1.242-.514 2.402-.566 3.48-.06 1.083.078 2.013.402 2.796.322.785.9 1.39 1.74 1.818.837.435 2.034.654 3.604.654 1.293 0 2.553-.123 3.77-.367a26.739 26.739 0 0 0 3.68-1.01l.895 9.235a43.015 43.015 0 0 1-5.264 1.535c-1.893.436-4.134.646-6.724.646-3.724 0-6.611-.553-8.668-1.654-2.054-1.109-3.506-2.624-4.375-4.542-.857-1.91-1.24-4.114-1.133-6.596.11-2.488.486-5.103 1.133-7.857l7.94-33.527zM164.953 45.855c0-3.669.594-7.129 1.78-10.368 1.186-3.242 2.893-6.077 5.108-8.51 2.207-2.42 4.896-4.339 8.06-5.747 3.15-1.4 6.678-2.106 10.565-2.106 2.433 0 4.606.23 6.518.691 1.92.465 3.657 1.066 5.228 1.82l-4.134 9.4a38.71 38.71 0 0 0-3.36-1.174c-1.16-.357-2.576-.529-4.251-.529-4.001 0-7.164 1.38-9.518 4.128-2.345 2.751-3.526 6.454-3.526 11.1 0 2.752.594 4.978 1.786 6.681 1.186 1.703 3.377 2.55 6.558 2.55 1.57 0 3.085-.164 4.536-.484 1.462-.324 2.753-.732 3.89-1.214l.895 9.636a43.381 43.381 0 0 1-5.022 1.584c-1.838.45-4.026.682-6.563.682-3.35 0-6.184-.49-8.503-1.455-2.32-.98-4.237-2.28-5.747-3.929-1.518-1.652-2.608-3.58-3.282-5.795a23.772 23.772 0 0 1-1.018-6.96zM218.203 63.995c-2.861 0-5.346-.436-7.454-1.299-2.102-.863-3.843-2.074-5.22-3.644-1.38-1.562-2.411-3.413-3.118-5.546-.707-2.132-1.047-4.493-1.047-7.08 0-3.245.52-6.489 1.574-9.724 1.048-3.242 2.603-6.155 4.66-8.744 2.043-2.593 4.562-4.713 7.528-6.366 2.963-1.642 6.37-2.468 10.199-2.468 2.809 0 5.28.437 7.418 1.3 2.127.861 3.879 2.082 5.264 3.644 1.37 1.57 2.41 3.413 3.11 5.55.706 2.127 1.054 4.494 1.054 7.083 0 3.235-.514 6.48-1.534 9.724-1.021 3.23-2.536 6.15-4.536 8.744-1.996 2.59-4.492 4.708-7.49 6.354-2.994 1.646-6.466 2.472-10.408 2.472zm5.99-34.662c-1.776 0-3.347.516-4.692 1.535-1.35 1.031-2.484 2.327-3.398 3.89-.924 1.57-1.61 3.282-2.072 5.143-.46 1.865-.684 3.628-.684 5.303 0 2.703.436 4.808 1.293 6.323.869 1.507 2.43 2.265 4.699 2.265 1.783 0 3.346-.512 4.699-1.542 1.342-1.023 2.477-2.32 3.398-3.886.918-1.562 1.609-3.279 2.072-5.143.453-1.859.684-3.632.684-5.304 0-2.696-.434-4.806-1.3-6.319-.863-1.507-2.431-2.265-4.698-2.265zM255.233 62.865h-11.997l10.123-42.597h12.075l-10.201 42.597zm5.824-47.787a7.424 7.424 0 0 1-4.532-1.496c-1.35-.992-2.028-2.519-2.028-4.577 0-1.132.233-2.194.69-3.196a8.442 8.442 0 0 1 1.824-2.593 8.95 8.95 0 0 1 2.632-1.743 7.976 7.976 0 0 1 3.194-.645c1.676 0 3.19.498 4.538 1.496 1.349 1.003 2.03 2.53 2.03 4.577 0 1.136-.242 2.202-.695 3.202s-1.062 1.861-1.817 2.593a8.894 8.894 0 0 1-2.63 1.738c-1.004.437-2.068.644-3.206.644zM274.073 22.205c.91-.266 1.926-.586 3.03-.934a40.19 40.19 0 0 1 3.733-.964c1.369-.3 2.914-.545 4.613-.734 1.699-.193 3.635-.287 5.786-.287 6.322 0 10.68 1.841 13.086 5.512 2.404 3.671 2.82 8.695 1.26 15.063l-5.514 23H288l5.344-22.516c.326-1.406.582-2.765.77-4.093.192-1.316.18-2.476-.042-3.48a4.168 4.168 0 0 0-1.494-2.433c-.791-.619-1.986-.93-3.607-.93a24.36 24.36 0 0 0-4.776.492l-7.857 32.96h-12.071l9.805-40.656z"></path>
                                    </svg>
                                </div>
                            </div>

                            {{--
                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d unitpay" data-payment_id="45">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/Y-Money.png">
                                </div>
                            </div>


                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d unitpay" data-payment_id="45">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/unionpay.png">
                                </div>
                            </div>
                            --}}

                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="52">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/USDT.png">
                                </div>
                            </div>
                            <div style="position: relative;"  class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d"  data-payment_id="53">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <svg width="150" height="32" viewBox="0 0 150 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M2 22L12 27L8.5 22.5L12.5 11.5H18.5L22.5 23L18.5 27L14 31.5L29.5 24L21 0L18.5 6.5H13L10 0L2 22Z" fill="#F21818" />
                                        <path d="M76.9 25.5H81.5L78 22.1H76.9C76.5372 22.1 76.2402 21.9771 75.9816 21.7184C75.7229 21.4598 75.6 21.1628 75.6 20.8V6.5H72.2V20.8C72.2 21.6658 72.4042 22.4647 72.8185 23.1852L72.8223 23.1917L72.8262 23.198C73.253 23.8916 73.8233 24.4524 74.5307 24.8768L74.5388 24.8815C75.2577 25.2948 76.0482 25.5 76.9 25.5Z" fill="#F21818" />
                                        <path d="M111.003 25.5H116.016L106.713 17.9188L114.785 11.3H109.942L101.887 18.027V6.50003H98.4871V25.5H101.887V21.8768L104.174 20.0041L111.003 25.5Z" fill="#F21818" />
                                        <path d="M139.407 25.5H136.007V14.7H132.407V11.3H136.007V6.50003H139.407V11.3H150V14.7H139.407V25.5Z" fill="#F21818" />
                                        <path fill-rule="evenodd" clip-rule="evenodd" d="M70.3552 18.3652C69.9444 19.0781 69.3802 19.6502 68.6692 20.0767L68.666 20.0787C67.9588 20.4958 67.1657 20.7 66.3 20.7H58.001C58.2432 21.0092 58.5342 21.28 58.8757 21.5135C59.4624 21.904 60.1165 22.1 60.852 22.1H70V25.5H60.852C59.877 25.5 58.9548 25.32 58.0904 24.957L58.0791 24.9523C57.2397 24.5754 56.4934 24.0605 55.8424 23.4096L55.8377 23.4048C55.2027 22.7526 54.7045 22.0051 54.3444 21.165L54.343 21.1616C53.9799 20.2972 53.8 19.375 53.8 18.4C53.8 17.4257 53.9797 16.5108 54.3437 15.6608C54.7033 14.8051 55.2015 14.0487 55.8378 13.3952L55.8424 13.3904C56.4945 12.7384 57.2428 12.2306 58.0849 11.8694C58.9504 11.4889 59.8746 11.3 60.852 11.3H66.3C67.1672 11.3 67.9609 11.513 68.6677 11.9463C69.3791 12.3568 69.9433 12.9209 70.3538 13.6324C70.787 14.3391 71 15.1328 71 16C71 16.8543 70.7856 17.6462 70.3552 18.3652ZM59.1483 15.1185C59.6632 14.8407 60.2281 14.7 60.852 14.7H66.3C66.6628 14.7 66.9598 14.8229 67.2184 15.0815C67.4771 15.3402 67.6 15.6371 67.6 16C67.6 16.3628 67.4771 16.6598 67.2184 16.9184C66.9598 17.1771 66.6628 17.3 66.3 17.3H57.3688C57.4815 16.9287 57.6507 16.5842 57.8765 16.2643C58.2171 15.7819 58.6398 15.4014 59.1483 15.1185Z" fill="#F21818" />
                                        <path d="M49.336 25.5V18.3H36.5V14.9H49.336V7.7H52.736V25.5H49.336Z" fill="#F21818" />
                                        <path d="M34 25.5H37.4V7.7H34V25.5Z" fill="#F21818" />
                                        <path fill-rule="evenodd" clip-rule="evenodd" d="M96.0552 18.3652C95.6444 19.0781 95.0802 19.6502 94.3692 20.0767L94.366 20.0787C93.6588 20.4958 92.8657 20.7 92 20.7H83.701C83.9432 21.0092 84.2342 21.28 84.5757 21.5135C85.1624 21.904 85.8165 22.1 86.552 22.1H95.7V25.5H86.552C85.577 25.5 84.6548 25.32 83.7904 24.957L83.7791 24.9523C82.9397 24.5754 82.1934 24.0605 81.5424 23.4096L81.5377 23.4048C80.9027 22.7526 80.4045 22.0051 80.0444 21.165L80.043 21.1616C79.6799 20.2972 79.5 19.375 79.5 18.4C79.5 17.4257 79.6797 16.5108 80.0437 15.6608C80.4033 14.8051 80.9015 14.0487 81.5378 13.3952L81.5424 13.3904C82.1945 12.7384 82.9428 12.2306 83.7849 11.8694C84.6504 11.4889 85.5746 11.3 86.552 11.3H92C92.8672 11.3 93.6609 11.513 94.3677 11.9463C95.0791 12.3568 95.6433 12.9209 96.0538 13.6324C96.487 14.3391 96.7 15.1328 96.7 16C96.7 16.8543 96.4856 17.6462 96.0552 18.3652ZM84.8483 15.1185C85.3632 14.8407 85.9281 14.7 86.552 14.7H92C92.3629 14.7 92.6598 14.8229 92.9184 15.0815C93.1771 15.3402 93.3 15.6371 93.3 16C93.3 16.3628 93.1771 16.6598 92.9184 16.9184C92.6598 17.1771 92.3629 17.3 92 17.3H83.0688C83.1815 16.9287 83.3507 16.5842 83.5765 16.2643C83.9171 15.7819 84.3398 15.4014 84.8483 15.1185Z" fill="#F21818" />
                                        <path fill-rule="evenodd" clip-rule="evenodd" d="M130.555 18.3652C130.144 19.0781 129.58 19.6502 128.869 20.0767L128.866 20.0787C128.159 20.4958 127.366 20.7 126.5 20.7H118.201C118.443 21.0092 118.734 21.28 119.076 21.5135C119.662 21.904 120.317 22.1 121.052 22.1H130.2V25.5H121.052C120.077 25.5 119.155 25.32 118.29 24.957L118.279 24.9523C117.44 24.5754 116.693 24.0605 116.042 23.4096L116.038 23.4048C115.403 22.7526 114.904 22.0051 114.544 21.165L114.543 21.1616C114.18 20.2972 114 19.375 114 18.4C114 17.4257 114.18 16.5108 114.544 15.6608C114.903 14.8051 115.401 14.0487 116.038 13.3952L116.042 13.3904C116.694 12.7384 117.443 12.2306 118.285 11.8694C119.15 11.4889 120.075 11.3 121.052 11.3H126.5C127.367 11.3 128.161 11.513 128.868 11.9463C129.579 12.3568 130.143 12.9209 130.554 13.6324C130.987 14.3391 131.2 15.1328 131.2 16C131.2 16.8543 130.986 17.6462 130.555 18.3652ZM119.348 15.1185C119.863 14.8407 120.428 14.7 121.052 14.7H126.5C126.863 14.7 127.16 14.8229 127.418 15.0815C127.677 15.3402 127.8 15.6371 127.8 16C127.8 16.3628 127.677 16.6598 127.418 16.9184C127.16 17.1771 126.863 17.3 126.5 17.3H117.569C117.682 16.9287 117.851 16.5842 118.076 16.2643C118.417 15.7819 118.84 15.4014 119.348 15.1185Z" fill="#F21818" />
                                    </svg>
                                    <div style="position: absolute; top: 0; right: 0; width: 40px; height: 20px; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: bold; color: #fff; padding: 10px 30px; background-color: #F21818; border-radius: 2px;">
                                        CRYPTO
                                    </div>

                                </div>
                            </div>

                            <!-- <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="46">
                    <div
                            class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM"
                            fill="inherit"
                    >
                      <div style="display: flex; align-items: center; justify-content: center; height: 100%; font-weight: bold; color: #26a17b; font-size: 12px;">
                        Crypto
                      </div>
                    </div>
                  </div> -->


                            {{--
                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d unitpay" data-payment_id="451">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/MasterCard KZT.png" alt="steam logo" style="width: 100%;">
                                </div>
                            </div>
                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d unitpay" data-payment_id="451">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/MasterCard UAH.png" alt="steam logo" style="width: 100%;">
                                </div>
                            </div>
                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d unitpay" data-payment_id="451">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/MasterCard USD.png" alt="steam logo" style="width: 100%;">
                                </div>
                            </div>
                            --}}

                            {{--
                            <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d" data-payment_id="46">
                                <div class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM" fill="inherit">
                                    <img src="/images/payment-logos/MasterCard USD.png" alt="Cryptocloud logo" style="width: 100%;">
                                </div>
                            </div>
                            --}}

                            <!-- <div class="styles__Method-sc-ena6u4-3 payment-item gcQxa-d tebex" data-payment_id="48">
                    <div
                      class="styles__Icon-sc-h5vgtd-0 gpvUmb styles__MethodIcon-sc-ena6u4-4 kDTdJM"
                      fill="inherit"
                    >
                      <span style="font-weight: bold;">{{ __('ДРУГИЕ...') }}</span>
                    </div>
                  </div> -->


                        </div>
                    </div>
                    <div class="ow__balance">

                        {{--
                        <div class="ow__balance--or">Или</div>
                        <div class="form-group">
                            <input type="checkbox" id="html" />
                            <label for="html">
                                <div class="ow-balance__box">
                                    <img src="/images/rustresort_icon.png" alt="icon" />
                                    <div class="ow-balance__text">
                                        <div class="ow-balance-text__top">
                                            Оплатить с помощью личного баланса
                                        </div>
                                        <div class="ow-balance-text__bottom">
                                            У вас на счету:
                                            <span>{{ getCurrentUserBalance() }}</span>
                                        </div>
                                    </div>
                                </div>
                            </label>
                        </div>
                        --}}

                    </div>
                    <div class="buying-item-terms">
                        <div>
                            <input type="checkbox" name="term" autocomplete="off"><label>{{ __('Я прочитал и согласен с') }} <a href="{{ route('term') }}" target="_blank">{{ __('Условия Обслуживания') }}</a></label>
                        </div>
                        <div>
                            <input type="checkbox" name="policy" autocomplete="off"><label>{{ __('Я прочитал и согласен с') }} <a href="{{ route('policy') }}" target="_blank">{{ __('Соглашение о Политике') }}</a></label>
                        </div>
                    </div>
                    <div class="styles__BalanceBlock-sc-n3x62l-0 yqtHU">
                        <div class="styles__FlexContainer-sc-n3x62l-17 dgiuLo">
                            <div class="styles__Label-sc-n3x62l-14 ehTfkQ">
                                {{ __('Промокод для бонуса на баланс') }}:
                            </div>
                            <div class="styles__CouponDescription-sc-n3x62l-18 liVkaJ"></div>
                        </div>

                        @if(Session::get('deposit_bonus', 0) > 0)
                            <div class="styles__FlexContainer-sc-n3x62l-17 dgiuLo promocode-applied">
                                {{ __('Промокод успешно активирован!') . ' ' . __('Вы получите') . ' +' . Session::get('deposit_bonus', 0) . '% ' . __('при пополнении баланса') }}
                            </div>
                        @else
                            <div class="styles__FlexContainer-sc-n3x62l-17 dgiuLo">
                                <div class="styles__CouponSection-sc-n3x62l-16 kwIswO">
                                    <input autocomplete="off" id="promocode" name="promocode" placeholder="{{ __('Введите промокод') }}" spellcheck="false" type="text" value="" />
                                </div>
                                <div class="styles__ApplyButton-sc-n3x62l-15 bxJyxD" id="apply-promocode">
                                    {{ __('Применить') }}
                                </div>
                            </div>
                        @endif

                        <div class="styles__FlexContainer-sc-n3x62l-19 dgiuLo unipay-fields" style="display: none">
                            <div class="styles__CouponSection-sc-n3x62l-16 kwIswO">
                                <input autocomplete="off" id="email" name="email" placeholder="{{ __('Введите Email') }}" spellcheck="false" type="text" value="" />
                            </div>
                        </div>


                        <div class="styles__MoneyBlock-sc-n3x62l-10 bgUbWn">
                            <div class="styles__Money-sc-n3x62l-11 bSqOVl">
                                <div class="styles__Label-sc-n3x62l-14 ehTfkQ">
                                    {{ __('Сумма пополнения') }}
                                </div>
                                <div class="styles__PaymentInfo-sc-n3x62l-12 bERcWv">
                                    <input tabindex="0" type="number" id="amount" name="amount" class="styles__PaymentInput-sc-n3x62l-13 DGJNK price" min="1" step="1" max="9999" value="1" />
                                    <div class="styles__Icon-sc-h5vgtd-0 emYpJf styles__Rouble-sc-qhub7g-0 kozvEy">
                                        @if(app()->getLocale() == 'ru'){{ '₽' }}@else{{ '$' }}@endif
                                    </div>
                                </div>
                            </div>
                            <div class="styles__Money-sc-n3x62l-11 bSqOVl">
                                <div class="styles__Label-sc-n3x62l-14 ehTfkQ">
                                    {{ __('Получите на баланс') }}
                                </div>
                                <div class="styles__PaymentInfo-sc-n3x62l-12 bERcWv">
                                    <input readonly="" tabindex="0" id="amount-total" class="styles__PaymentInput-sc-n3x62l-13 DGJNK" value="1" />
                                    <div class="styles__Icon-sc-h5vgtd-0 emYpJf styles__Rouble-sc-qhub7g-0 kozvEy">
                                        @if(app()->getLocale() == 'ru'){{ __('₽') }}@else{{ '$' }}@endif

                                    </div>
                                </div>
                            </div>

                            <div class="bSqOVl tebex-text" style="display: none;">
                                <div style="display: flex;align-items: center;height: 100%;font-size: 14px;">
                                    {{ __('Там вам предложат приобрести наши услуги') }}.
                                </div>
                            </div>

                            <div class="bSqOVl alfa-text" style="display: none;">
                                <div style="display: flex;align-items: center;height: 100%;font-size: 14px;color: red;font-weight: bold;padding: 0 5px;">
                                    {{ __('Оплата с помощью MasterCard и СБП , временно не доступна. Осуществляются изменения в приёме платежей, ориентировочное время восстановления работы 3-ое суток. Вы можете связаться с тех. поддержкой в нашем дискорд канале что бы уточнить информацию: https://discord.gg/rustresort') }}
                                </div>
                            </div>

                            <div id="btn-buy" class="styles__ApplyButton-sc-ena6u4-10 fyAHtA">
                                {{ __('Пополнить баланс') }}
                            </div>
                            <div class="bSqOVl steam-text" style="display: none;">
                                <div style="display: flex;align-items: center;height: 100%;font-size: 14px;">
                                    {{ __('Сумма пополнения зависит от общей стоимости отправленных предметов на обмен') }}. {{ __('К пополнению доступны скины Rust и Dota 2') }}
                                </div>
                            </div>
                            <a href="https://steamcommunity.com/tradeoffer/new/?partner=1896100486&token=-G-Xcb1q" id="btn-buy-steam" class="styles__ApplyButton-sc-ena6u4-10 fyAHtA" style="display: none;text-decoration: none;">
                                {{ __('Пополнить баланс') }}
                            </a>
                        </div>

                    </div>

                </form>

                <form method="POST" action="{{ route('promocode.activate') }}" enctype="multipart/form-data" id="promocode-form">
                    @csrf
                    <input type="hidden" id="code" name="code" value="">

                </form>
            </div>
        </div>
    </div>

@endsection

@push('scripts')
    <script src="/js/stats.js"></script>

    <script>
        $('.payment-item').on('click', function () {
            $('#payment_id').val($(this).data('payment_id'));

            if ($(this).hasClass('steam')) {
                $('.styles__FlexContainer-sc-n3x62l-17').hide();
                $('.styles__Money-sc-n3x62l-11').hide();
                $('#btn-buy').hide();
                $('#btn-buy-tebex').hide();
                $('#btn-buy-steam').show();
                $('.tebex-text').hide();
                $('.steam-text').show();
                $('.unipay-fields').hide();
                $('.alfa-text').hide();
                $('.unipay-fields input').attr('required', false);
                $('#btn-buy').text('{{ __('Пополнить баланс') }}');
            } else if ($(this).hasClass('unitpay')) {
                $('.styles__FlexContainer-sc-n3x62l-17').show();
                $('.styles__Money-sc-n3x62l-11').show();
                $('#btn-buy').show();
                $('#btn-buy-steam').hide();
                $('.steam-text').hide();
                $('.tebex-text').hide();
                $('.alfa-text').hide();
                $('.unipay-fields').show();
                $('.unipay-fields input').attr('required', true);
                $('#btn-buy').text('{{ __('Пополнить баланс') }}');
            } else if ($(this).hasClass('tebex_OFF')) {
                $('.styles__FlexContainer-sc-n3x62l-17').hide();
                $('.styles__Money-sc-n3x62l-11').hide();
                $('#btn-buy').show();
                $('#btn-buy-steam').hide();
                $('.tebex-text').show();
                $('.steam-text').hide();
                $('.alfa-text').hide();
                $('.unipay-fields').hide();
                $('.unipay-fields input').attr('required', false);
                $('#btn-buy').text('{{ __('Открыть магазин') }}');
            } else if ($(this).hasClass('tebex')) {
                $('.styles__FlexContainer-sc-n3x62l-17').show();
                $('.styles__Money-sc-n3x62l-11').show();
                $('#btn-buy').show();
                $('#btn-buy').text('{{ __('Пополнить баланс') }}');
                $('#amount').val('100');
                $('#amount-total').val('100');
                $('.alfa-text').hide();
            } else if ($(this).hasClass('alfa_OFF')) {
                $('.styles__FlexContainer-sc-n3x62l-17').hide();
                $('.styles__Money-sc-n3x62l-11').hide();
                $('#btn-buy').hide();
                $('#btn-buy-steam').hide();
                $('.tebex-text').hide();
                $('.steam-text').hide();
                $('.alfa-text').show();
                $('.unipay-fields').hide();
            } else {
                $('.styles__FlexContainer-sc-n3x62l-17').show();
                $('.styles__Money-sc-n3x62l-11').show();
                $('#btn-buy').show();
                $('#btn-buy-steam').hide();
                $('.steam-text').hide();
                $('.tebex-text').hide();
                $('.alfa-text').hide();
                $('.unipay-fields').hide();
                $('.unipay-fields input').attr('required', false);
                $('#btn-buy').text('{{ __('Пополнить баланс') }}');
            }

        });

        $('#email').on('click', function () {
            $('.unipay-fields').removeClass('check-error');
        });

        $('#btn-buy').on('click', function () {
            if ($('#payment_id').val() === "") {
                return;
            }

            $('input[name="term"]').removeClass('check-error');
            $('input[name="policy"]').removeClass('check-error');

            if (!$('input[name="term"]').is(':checked')) {
                $('input[name="term"]').addClass('check-error');
                return false;
            }
            if (!$('input[name="policy"]').is(':checked')) {
                $('input[name="policy"]').addClass('check-error');
                return false;
            }
            if (($('#payment_id').val() == '45' || $('#payment_id').val() == '451') && $('#email').val() == '') {
                $('.unipay-fields').addClass('check-error');
                return false;
            }
            $('#topup-balance-form').submit();
        });

        $('#apply-promocode').on('click', function () {
            $('#code').val($('#promocode').val());
            $('#promocode-form').submit();
        });

        $('#amount').on('change', function () {
            let amount = 0;
            @if(Session::get('deposit_bonus', 0) > 0)
                amount = parseFloat({{ Session::get('deposit_bonus', 1) }} * $(this).val() / 100) + parseFloat($(this).val());
            @else
                amount = $(this).val();
            @endif
            $('#amount-total').val(amount);
        });

        $('#topup').on('click', function () {
            $('.balance-modal').show();
        });
        $('.select-stats .option').on('click', function () {
            console.log($(this).data('type'));
            location.href = "{{ route('stats') }}?type=" + $(this).data('type') + "&search=" + $('#search').val();
        });
        $('input[name="search"]').on('change', function () {
            console.log($(this).val());

            @if(isset($user->player_id))
                //location.href = "{{ route('account.stats', $user->player_id) }}?search=" + $(this).val();
            @elseif(isset(auth()->user()->steam_id))
                //location.href = "{{ route('account.stats', auth()->user()->steam_id) }}?search=" + $(this).val();
            @endif
        });

        @if(request()->query('topup') == '1')
            $('.balance-modal').show();
        @endif
    </script>
@endpush